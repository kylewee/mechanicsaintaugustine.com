...
?>
  <tr>
    <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/track_changes/entities_delete','reports_id=' . _get::int('reports_id'). '&id=' . $v['id'])) . ' ' . button_icon_edit(url_for('ext/track_changes/entities_form','reports_id=' . _get::int('reports_id'). '&id=' . $v['id'])); ?></td>        
    <td><?php echo $v['report_name'] ?></td>        
    ...
    {	
    	$fields_list = array();
    	$fields_query = db_query("select f.name from app_fields f, app_forms_tabs t where f.entities_id='" . $v['entities_id'] . "' and f.id in (" . $v['track_fields'] . ") and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
