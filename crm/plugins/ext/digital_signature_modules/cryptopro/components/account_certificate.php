...
if($cryptopro_certificates = db_fetch_array($cryptopro_certificates_query))
{
        ...
        if ($res) 
        {
            ...
            $html .= '
                    ...
                    <tr>
                        ...
                        <td>' . format_date(strtotime( (strlen($CertData['validFrom'])==13 ? '20':'') . $CertData['validFrom'])) . " до " . format_date(strtotime( (strlen($CertData['validFrom'])==13 ? '20':'') . $CertData['validTo'])) . '</td>
                    </tr>
                    <tr>
                        <td><hr></td>
                        <td><hr></td>
                    ...
                    <tr>
                        ...
                        <td>' . str_replace(":", "", str_replace("keyid:", "", $CertData['extensions']['subjectKeyIdentifier'])) . '</td>
                    </tr>
                    <tr>
                        <td>Идентификатор ключа УЦ:</td>
                        <td>' . substr(str_replace(":", "", str_replace("keyid:", "", $CertData['extensions']['authorityKeyIdentifier'])), 0, 40) . '</td>
                    </tr>
                    <tr>
                        <td><hr></td>
                        <td><hr></td>
    ...
    echo '<a href="' . url_for('users/signature_update'). '" class="btn btn-primary">Обновить сертификат</a> <a href="' . url_for('users/signature_account','action=delete'). '" onclick="return confirm(\'' . TEXT_ARE_YOU_SURE . '\')" class="btn btn-default">Удалить</a>';
}
else
{
