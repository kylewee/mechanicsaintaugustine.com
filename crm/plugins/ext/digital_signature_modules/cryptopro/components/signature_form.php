<?php echo ajax_modal_template_header('Просмотр данных перед подписанием') ?>

<?php echo form_tag('signature_form', url_for('items/digital_signature','action=sign&fields_id=' . _GET('fields_id') . '&path=' . $app_path)) ?>
...
<p>На данной форме отображается содержание информации, подписание которой производится  (в соответствии с п. 2 статьи 12 Федерального закона № 63-ФЗ от 6 апреля 2011г.).</p>

<?php

$item_query = db_query("select e.* " . fieldtype_formula::prepare_query_select($current_entity_id, '') . " from app_entity_" . $current_entity_id . " e where id='" . $current_item_id . "'",false);
$item = db_fetch_array($item_query);

$fields_access_schema = users::get_fields_access_schema($current_entity_id,$app_user['group_id']);

//sign item
$where_sql = " and f.id in ('" . implode("','",$cfg->get('signature_fields')). "') and f.type not in (" . fields_types::get_attachments_types_list(). ")";
...
{    
    ...
    <div class="tab-content">
      ...
      <div class="tab-pane fade" id="tab_sign_data_source">
      	<?php echo textarea_tag('sign_source',$sign_source,['class'=>'form-control','style'=>'height:250px;','readonly'=>'readonly'])?>
      	<?php echo input_hidden_tag('sign_source_base64',base64_encode($sign_source),['class'=>'data-to-sign','signature_container'=>'sign_source_signature']) ?>      	
      	<?php echo input_hidden_tag('sign_source_signature','',['class'=>'form-control','placeholder'=>'ЭЦП']) ?>
      	<br>
      </div>
    </div>
...
//sign attachments
$where_sql = " and f.id in ('" . implode("','",$cfg->get('signature_fields')). "') and f.type in (" . fields_types::get_attachments_types_list(). ")";
$fields_query = fields::get_query($current_entity_id,$where_sql);
if(db_num_rows($fields_query))
{
    $attachments_list = [];
    ...
    while($fields = db_fetch_array($fields_query))
    {
        ...
        if(strlen($value))
        {
            foreach(explode(',',$value) as $key=>$filename)
            {
                ...
                $link = link_to($file['name'],url_for('items/info','path=' . $app_path . '&action=download_attachment&file=' . urlencode(base64_encode($filename))) . '&field=' . $fields['id'] );
                
                $link .= input_hidden_tag('sign_file_name[' . $key. ']',$filename);
                $link .= '<div class="hidden">' . input_tag('sign_file_base64[' . $key. ']',base64_encode(file_get_contents($file['file_path'])),['class'=>'data-to-sign','signature_container'=>'sign_file_signature_' . $key,'disabled'=>'disabled']) . '</div>';                
                $link .= input_hidden_tag('sign_file_signature[' . $key . ']','',['class'=>'form-control','placeholder'=>'ЭЦП']);
...
$user_certificate = '';
$cryptopro_certificates_query = db_query("select thumbprint from app_ext_cryptopro_certificates where users_id='" . $app_user['id']. "'");
  ...
  <div id="left-col1">
     <div id="info">
        <div id="info_msg" style="text-align:center;">
           ...
           <img src="images/red_dot.png" width="10" height="10" alt="Плагин не загружен" id="PluginEnabledImg"/>
     ...
     <p>
     	<select name="CertListBox" id="CertListBox" class="form-control" placeholder="Выберите сертификат"> </select>
     </p>  
          
     <div id="cert_info" style="display:none">
 		    
        <ul class="nav nav-tabs">
          <li class="active"><a href="#tab_sign_attachments"  data-toggle="tab">Информация о сертификате</a></li>        
        </ul>
    ...
    <div style="padding-bottom: 10px;"> 	
    	<button type="button" class="btn btn-primary" style="display:none" id="sign_process_btn" onClick="sign_process()"><i class="fa fa-pencil"></i> Подписать</button>
     ...
     <div style="display:none">
     	...
     	<input id="DataInBase64" name="DataInBase64" type="checkbox" checked="checked"> <!-- Данные уже закодированы в base64 -->
     </div>  
      
     <textarea id="DataToSignTxtBox" name="DataToSignTxtBox" class="form-control hidden" placeholder="текст для подписания"></textarea>
